...
To count per manufacturer, remove 'DISTINCT'
by=manufacturer is also not correct; grouping variable not really available for aggregation; instead

(this is not quite right...)

```{r, tidy=FALSE}
#manu <- data.table(dbGetQuery(geo_con, 
#    "SELECT manufacturer FROM gpl ORDER BY manufacturer ASC;"))
#manu[,list(, list(N), by = manufacturer]
```
...
res2 <- merge(gse[gse_gpl, nomatch = 0], gpl, by = "gpl")[(title %like% "HCV" | title %like% "%epatit") 
...
ebay1 <- eBayes(fit1)  # regularized t-test
...
# BH is default
tt1 <- topTable(ebay1, coef = "treatmentPoly", number = 50,  p.value = 0.01, lfc = log2(1.5), sort.by = "p")

# number above is # of genes to look at
...
```{r}
library(ggplot2)
ggplot(md_long_short, aes(x = treatment, y = expression_value)) + geom_violin() + geom_jitter(aes(color = infectionStatus)) + facet_wrap( ~Symbol + probe_name, scales = "free")


ggplot(md_long_short, aes(x = treatment, y = expression_value)) + geom_boxplot() + geom_jitter(aes(color = infectionStatus)) + facet_wrap( ~Symbol + probe_name, scales = "free")
```



To plot one gene vs another, we will need to reshape (right now the data is in a very long format)
using dcast (kind of the opposite of melt)

We want the columns to vary across probe names

```{r}
md_long_short2 <- md_long_short[, probe_name %in% c("ILMN_1657871", "ILMN_1701789", "ILMN_1707695"), list(probe_name, Symbol, ptid, cellType, infectionStatus, treatment, expression_value)]

md_short2 <- dcast(md_long_short2, formula = ptid + cellType + infectionStatus + treatment ~ probe_name) 

md_Short3 <- melt(md_short2, id.vars =  c("ptid", "cellType", "infectionStatus", "treatment", "ILMN_1701789"))
```
